...
export default function AnimatedSplashScreen() {
  ...
  useEffect(() => {
    ...
    const ringConfig = { duration: 2500, easing: Easing.out(Easing.ease) };
    
    // Ring 1
    ringScale1.value = withRepeat(withTiming(3.5, ringConfig), -1, false);
    ringOpacity1.value = withRepeat(withTiming(0, ringConfig), -1, false);

    // Ring 2 (Delayed)
    const timer = setTimeout(() => {
      ringScale2.value = withRepeat(withTiming(3.5, ringConfig), -1, false);
      ringOpacity2.value = withRepeat(withTiming(0, ringConfig), -1, false);
    }, 1250);

    // Futuristic fluid progress bar loading
    ...
    <View style={styles.container}>
        ...
        <View style={styles.animationArea}>
          ...
          <View style={styles.pinContainer}>
            <Animated.View style={[styles.pin, pinAnimStyle]}>
              ...
              >
                <MaterialCommunityIcons name="moped-electric" size={32} color="#FFFFFF" />
              </LinearGradient>
            </Animated.View>
      ...
      <Animated.View entering={FadeIn.delay(1000).duration(800)} style={styles.progressContainer}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressText}>Initiating secure network...</Text>
        ...
        <View style={styles.badgesWrapper}>
          <View style={styles.badge}>
            <MaterialCommunityIcons name="lightning-bolt" size={13} color="#94A3B8" />
            <Text style={styles.badgeText}>Real-time</Text>
          </View>
          <View style={styles.badgeDot} />
          <View style={styles.badge}>
            <MaterialCommunityIcons name="map-marker-radius" size={13} color="#94A3B8" />
            <Text style={styles.badgeText}>Optimized Routing</Text>
          </View>
        </View>
      </Animated.View>
    </View>
...
const styles = StyleSheet.create({
  ...
  orbTop: {
    ...
    backgroundColor: 'rgba(59, 130, 246, 0.12)', // Subtle blue glow
    shadowColor: '#3B82F6',
    shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 1,
    shadowRadius: 100,
    elevation: 24, // Android equivalent for filter blur
  },
  orbBottom: {
    position: 'absolute',
    bottom: -height * 0.1,
